package br.udc.engenharia.agenda.application.aspect;
...
public class ExceptionHandlerAspect
{
	...
	/**
	 * Trata exce��es geradas pelo Hibernate antes de enviar para o banco
	 * 
	 * @param joinPoint
	 * @param exception
	 */
	...
	/**
	 * Trata exce��es de Constraint geradas pelo PostgreSQL
	 * 
	 * @param joinPoint
	 * @param exception
	 */
	...
	{
		// Caso a exce��o j� tenha sido interceptada por outro Aspecto deve ser ignorada
		...
		if ( exception.getCause() instanceof ConstraintViolationException )
		{
			...
			String key;
			// Verifica o c�digo do erro gerado pelo PostgreSQL
			switch ( cause.getSQLState() )
			{
				case "23503": // foreign_key_violation
				{
					key = message.substring( message.indexOf( '"' ) + 1, message.indexOf( '.' ) - 1 );
					throw new DataIntegrityViolationException( "N�o � poss�vel realizar a opera��o pois este registro � referenciado em " + key + "." );// FIXME Localize
				}
				case "23505": // unique_violation
				{
					key = message.substring( message.indexOf( '(' ) + 1, message.indexOf( ')' ) );
					throw new DataIntegrityViolationException( "O campo " + key + " informado j� existe. Altere e tente novamente." ); // FIXME Localize
				}
				...
				default:
				{
					throw new DataIntegrityViolationException( "N�o foi poss�vel realizar a opera��o pois ocorreu um problema de integridade (C�digo " + cause.getSQLState() + "). Verifique e tente novamente." ); // FIXME Localize
				}
			}
		}
