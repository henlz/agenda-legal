...
function AbstractCRUDController($scope, $log, $state, $rootScope, $timeout) {
    ...
    $scope.$watch('msg', function (newValue, oldValue) {
        $timeout(function () {
            if (newValue && newValue.text && newValue.text.length > 0 && $('.alert').length) {

                //Remove a mensagem de alerta apenas quando der um click na mensagem
//              angular.element( $('.alert') ).bind('click', function(){
//                  angular.element( $('.alert') ).scope().$parent.msg = null;
//                  angular.element( $('.alert') ).unbind('click');
//                  $scope.$apply();
//              });

                //Remove a mensagem de alerta ao dar um click em qualquer area da tela
