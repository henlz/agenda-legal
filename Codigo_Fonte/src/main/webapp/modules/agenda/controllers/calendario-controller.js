(function (angular) {
    ...
    angular.module('agenda').controller('CalendarioController', function ($scope, $compile, $importService, $log, $state, $mdToast, $mdDialog, $timeout) {
            ...
            $scope.SEARCH_STATE = "agenda.pesquisar";
            ...
            $scope.initialize = function (toState, toParams, fromState, fromParams) {
                ...
                switch (state) {
                    ...
                    case $scope.UPDATE_STATE:
                    {
                        ...
                        break;
                    case $scope.SEARCH_STATE:
                    {
                        $scope.changeToSearch();
                    }
                        break;
                    default :
                    {
            ...
            $scope.changeToSearch = function () {
                $scope.currentState = $scope.SEARCH_STATE;
             ...
             *
             */
            $scope.pesquisarCompromissos = function () {
                var tipoCompromissoId = $scope.filtros.tipoCompromisso == null ? null : $scope.filtros.tipoCompromisso.id;
                var categoriaCompromissoId = $scope.filtros.categoriaCompromisso == null ? null : $scope.filtros.categoriaCompromisso.id;

                compromissoService.listAgendasByFilters($scope.filtros.nome, $scope.filtros.dataInicio, $scope.filtros.dataFim,
                    tipoCompromissoId, categoriaCompromissoId, {
                    callback: function (result) {
                        $scope.listaCompromissos = result;
                        $scope.$apply();
                    },
                    errorHandler: function (message, error) {
                        $log.error(message);
             ...
             *
             */
            $scope.gridSearchOptions = {
                data: 'listaCompromissos',
                columnDefs: [
                    {displayName: "Titulo", field: "compromisso.titulo", width: "60%"},
